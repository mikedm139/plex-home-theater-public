//
//  GUIWindowPlexVideoPlaylist.cpp
//  Plex
//
//  Created by mikedm139 (https://github.com/mikedm139) on 2013-10-19
//  based on GuiWindowVideoPlaylist.cpp by
...
#include "GUIWindowPlexVideoPlaylist.h"
...
CGUIWindowPlexVideoPlaylist::CGUIWindowPlexVideoPlaylist()
: CGUIPlexMediaWindow(WINDOW_VIDEO_PLAYLIST, "MyVideoPlaylist.xml")
{
  m_movingFrom = -1;
}
...
bool CGUIWindowPlexVideoPlaylist::OnMessage(CGUIMessage& message)
{
  ...
  case GUI_MSG_WINDOW_INIT:
    {
      ...
      if (!CGUIPlexMediaWindow::OnMessage(message))
        return false;
...
bool CGUIWindowPlexVideoPlaylist::OnAction(const CAction &action)
{
...
bool CGUIWindowPlexVideoPlaylist::OnBack(int actionID)
{
  if (actionID == ACTION_NAV_BACK)
    return CGUIWindow::OnBack(actionID); // base class goes up a folder, but none to go up
  return CGUIPlexMediaWindow::OnBack(actionID);
}

bool CGUIWindowPlexVideoPlaylist::MoveCurrentPlayListItem(int iItem, int iAction, bool bUpdate /* = true */)
{
  int iSelected = iItem;
  int iNew = iSelected;
...
void CGUIWindowPlexVideoPlaylist::ClearPlayList()
{
  ClearFileItems();
...
void CGUIWindowPlexVideoPlaylist::UpdateButtons()
{
  ...
  CGUIPlexMediaWindow::UpdateButtons();
...
bool CGUIWindowPlexVideoPlaylist::OnPlayMedia(int iItem)
{
...
void CGUIWindowPlexVideoPlaylist::RemovePlayListItem(int iItem)
{
...
void CGUIWindowPlexVideoPlaylist::SavePlayList()
{
...
void CGUIWindowPlexVideoPlaylist::GetContextButtons(int itemNumber, CContextButtons &buttons)
{
...
bool CGUIWindowPlexVideoPlaylist::OnContextButton(int itemNumber, CONTEXT_BUTTON button)
{
  switch (button)
  {
  ...
  return CGUIPlexMediaWindow::OnContextButton(itemNumber, button);
}

void CGUIWindowPlexVideoPlaylist::OnMove(int iItem, int iAction)
{
  if (iItem < 0 || iItem >= m_vecItems->Size()) return;
  MoveCurrentPlayListItem(iItem, iAction);
}

void CGUIWindowPlexVideoPlaylist::MoveItem(int iStart, int iDest)
{
...
void CGUIWindowPlexVideoPlaylist::MarkPlaying()
{
  /*  // clear markings
